...
if (empty($_SERVER['HTTP_X_REQUESTED_WITH']) || $_SERVER['HTTP_X_REQUESTED_WITH'] != 'XMLHttpRequest') {
    $output['result'] = 'Ошибка 1';
    exit(json_encode($output));    
}

if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    $output['result'] = 'Ошибка 2';
    exit(json_encode($output));   
}
...
switch ($_POST['action']) {
    ...
    //debug($_POST['rating']);
        ...
        if ($count == 0) {
            ...
            // echo('Записываем:'.$ratingId.' '.$ratingAvg.' '.$totalVotes);
            // debug("Вставка");
            try {
